...
class NYUv2(Dataset):
    ...
    """
    def __init__(self, root, mode='train', augmentation=False, flip=0, flipped_task=None):
        self.mode = mode
        self.root = os.path.expanduser(root)
        self.augmentation = augmentation
        self.flip = flip 
        self.flipped_task = flipped_task
        ...
        self.flip_index = np.random.choice(data_len, int(data_len*self.flip), replace=False)
    
    """
        Specific to inserting noise into the task (ExcessMTL)
    """ 
    def __inject_noise__(self, semantic, depth, normal):

        if self.flipped_task == "semantic":
            # semantic -1 to 12
            semantic = torch.randint(-1, 13, semantic.shape)
            
        elif self.flipped_task == "depth":
            depth = torch.from_numpy(np.random.randn(depth.shape[0], depth.shape[1], depth.shape[2])).to(depth)

        elif self.flipped_task == "normal":
            normal[-1, :, :] = torch.from_numpy(np.random.randn(normal.shape[1], normal.shape[2])).to(normal)
            normal = normal / np.linalg.norm(normal, axis=0, keepdims=True)
        
        return semantic.float(), depth.float(), normal.float()
    ...
    def __getitem__(self, i):
        ...
        # specific to inserting noise into the task (ExcessMTL)
        if self.mode == 'train' and self.flip and index in self.flip_index:
            semantic, depth, normal = self.__inject_noise__(semantic, depth, normal)
