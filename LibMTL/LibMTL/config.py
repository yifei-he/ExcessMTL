...
_parser.add_argument('--weighting', type=str, default='EW',
    help='loss weighing strategies, option: EW, UW, GradNorm, GLS, RLW, \
        MGDA, PCGrad, GradVac, CAGrad, GradDrop, DWA, IMTL, ExcessMTL, GroupDRO')
...
## ExcessMTL
_parser.add_argument('--robust_step_size', default=1e-2, type=float, 
                    help='step size for GroupDRO update')
...
def prepare_args(params):
    ...
    if params.weighting in ['EW', 'UW', 'GradNorm', 'GLS', 'RLW', 'MGDA', 'IMTL',
                            ...
                            'Nash_MTL', 'MoCo', 'Aligned_MTL', 'DB_MTL', 'STCH', 'ExcessMTL', 'GroupDRO']:
        ...
        elif params.weighting in ['GroupDRO', 'ExcessMTL']:
            if params.robust_step_size is not None:
                kwargs['weight_args']['robust_step_size'] = params.robust_step_size
            else:
                raise ValueError(f'{params.weighting} needs keywaord robust_step_size')
    else:
